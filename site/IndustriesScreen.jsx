    ...
    <div>
      ...
      <Section>
        <div style={{ display: "grid", gridTemplateColumns: "0.8fr 1.2fr", gap: 48, alignItems: "start" }}>
          ...
          {/* Detail */}
          <div key={active} style={{ display: "flex", flexDirection: "column", gap: 24 }}>
            <Photo icon={s.icon} label={s.name.toUpperCase()} height={240} />
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              <Eyebrow>{s.name}</Eyebrow>
              <p style={{ margin: 0, fontSize: 17, lineHeight: 1.7, color: "var(--text-secondary)" }}>{s.overview}</p>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
              <div style={{ display: "flex", flexDirection: "column", gap: 12, padding: 24, background: "var(--gray-50)", border: "1px solid var(--border-subtle)", borderRadius: "var(--radius-lg)" }}>
                <span style={{ display: "inline-flex", alignItems: "center", gap: 8, fontFamily: "var(--font-heading)", fontSize: 15, fontWeight: 600, color: "var(--text-primary)" }}><Icon name="alert-triangle" size={17} color="var(--color-warning, #C77700)" /> Challenges</span>
                <ul style={{ margin: 0, padding: 0, listStyle: "none", display: "flex", flexDirection: "column", gap: 8 }}>
                  {s.challenges.map((c) => <li key={c} style={{ fontSize: 14.5, lineHeight: 1.5, color: "var(--text-secondary)", paddingLeft: 16, position: "relative" }}><span style={{ position: "absolute", left: 0, top: 8, width: 5, height: 5, borderRadius: "50%", background: "var(--text-muted)" }} />{c}</li>)}
                </ul>
              </div>
              <div style={{ display: "flex", flexDirection: "column", gap: 12, padding: 24, background: "var(--aqua-50)", border: "1px solid var(--aqua-200, rgba(0,183,199,0.3))", borderRadius: "var(--radius-lg)" }}>
                <span style={{ display: "inline-flex", alignItems: "center", gap: 8, fontFamily: "var(--font-heading)", fontSize: 15, fontWeight: 600, color: "var(--navy-800)" }}><Icon name="check-circle" size={17} color="var(--aqua-600)" /> Our Solutions</span>
                <ul style={{ margin: 0, padding: 0, listStyle: "none", display: "flex", flexDirection: "column", gap: 8 }}>
                  {s.solutions.map((c) => <li key={c} style={{ display: "flex", gap: 8, fontSize: 14.5, lineHeight: 1.5, color: "var(--navy-800)" }}><Icon name="check" size={15} color="var(--aqua-600)" style={{ marginTop: 2, flexShrink: 0 }} />{c}</li>)}
                </ul>
              </div>
            </div>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 16, padding: "20px 24px", border: "1px solid var(--border-subtle)", borderRadius: "var(--radius-lg)" }}>
