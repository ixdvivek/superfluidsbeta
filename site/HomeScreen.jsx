...
function HomeScreen({ onNavigate }) {
  const { Button, Eyebrow, SectionHeader, SolutionCard, FeatureBlock, StatCard, ProjectCard, ProcessTimeline, Testimonial, LogoBar, Badge, Icon } = DSh;
  ...
  return (
    <div>
      ...
      <section style={{ position: "relative", background: "var(--gradient-navy)", overflow: "hidden" }}>
        ...
        <img src="assets/logos/superfluids-mark.png" alt="" aria-hidden="true" style={{ position: "absolute", right: "4%", top: "6%", height: "92%", opacity: 0.5, filter: "drop-shadow(0 20px 60px rgba(0,183,199,0.35))", pointerEvents: "none" }} />
        <div style={{ position: "relative", maxWidth: "var(--container-max)", margin: "0 auto", padding: "120px 32px 110px" }}>
          <div style={{ maxWidth: 720, display: "flex", flexDirection: "column", gap: 26 }}>
            ...
            <h1 style={{ margin: 0, fontFamily: "var(--font-heading)", fontSize: 60, lineHeight: 1.05, fontWeight: 800, letterSpacing: "-0.02em", color: "var(--white)", textWrap: "balance" }}>
              Complete Water &amp; Fluid Engineering Solutions Across UAE
            </h1>
            <p style={{ margin: 0, fontSize: 19, lineHeight: 1.6, color: "var(--text-on-dark-secondary)", maxWidth: 600 }}>
            ...
            <div style={{ display: "flex", gap: 14, flexWrap: "wrap", marginTop: 6 }}>
              <Button variant="accent" size="lg" iconRight="arrow-right" onClick={() => onNavigate("Contact")}>Request Consultation</Button>
              <Button variant="outline" size="lg" onClick={() => onNavigate("Products")} style={{ background: "rgba(255,255,255,0.06)", color: "var(--white)", borderColor: "rgba(255,255,255,0.25)" }}>View Products</Button>
            </div>
            <div style={{ display: "flex", gap: 40, marginTop: 22, paddingTop: 26, borderTop: "1px solid rgba(255,255,255,0.12)" }}>
      ...
      <Section>
        <SectionHeader align="center" eyebrow="Core Solutions" title="Engineered fluid systems, end to end"
          description="Six core capability areas — specified, supplied, installed and maintained by one accountable engineering partner." style={{ marginBottom: 56 }} />
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 24 }}>
          {SOLUTIONS.map((s) => <SolutionCard key={s.title} {...s} onClick={(e) => { e.preventDefault(); onNavigate("Products"); }} />)}
        </div>
      </Section>

      {/* WHY SUPERFLUIDS — navy band */}
      ...
      <Section bg="var(--surface)">
        <SectionHeader align="center" eyebrow="Industries Served" title="Trusted across every sector"
          description="From residential towers to oil &amp; gas and district cooling, our systems perform where reliability is non-negotiable." style={{ marginBottom: 48 }} />
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16 }}>
          {INDUSTRIES.map((ind) => (
            <div key={ind.name} style={{ display: "flex", alignItems: "center", gap: 16, padding: "20px 24px", background: "var(--gray-50)", border: "1px solid var(--border-subtle)", borderRadius: "var(--radius-md)", transition: "var(--transition-base)", cursor: "pointer" }}
              ...
              <span style={{ display: "inline-flex", width: 44, height: 44, borderRadius: "var(--radius-md)", background: "var(--aqua-50)", alignItems: "center", justifyContent: "center", color: "var(--aqua-600)" }}>
                <Icon name={ind.icon} size={22} strokeWidth={1.75} />
              </span>
              <span style={{ fontFamily: "var(--font-heading)", fontSize: 17, fontWeight: 600, color: "var(--text-primary)" }}>{ind.name}</span>
            </div>
          ))}
        </div>
      </Section>
          ...
          {PROJECTS.map((p) => (
            <div key={p.name} style={{ display: "flex", flexDirection: "column", background: "var(--surface)", border: "1px solid var(--border-subtle)", borderRadius: "var(--radius-lg)", overflow: "hidden", boxShadow: "var(--shadow-card)" }}>
              <div style={{ position: "relative" }}>
                <Photo icon={p.icon} label={p.location.toUpperCase()} height={200} radius="0" />
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10, padding: 24 }}>
                <h3 style={{ margin: 0, fontFamily: "var(--font-heading)", fontSize: 20, fontWeight: 600, color: "var(--text-primary)" }}>{p.name}</h3>
      ...
      {/* TESTIMONIALS — navy */}
      <Section bg="var(--gradient-navy)">
        <SectionHeader invert align="center" eyebrow="Client Feedback" title="What our clients say" style={{ marginBottom: 48 }} />
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 24 }}>
      ...
      <Section bg="var(--surface)" py={0} style={{ paddingTop: 0, paddingBottom: 96 }}>
        <div style={{ position: "relative", overflow: "hidden", borderRadius: "var(--radius-2xl)", background: "var(--gradient-fluid)", padding: "72px 64px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 40, flexWrap: "wrap" }}>
          ...
          <div style={{ position: "relative", display: "flex", flexDirection: "column", gap: 12, maxWidth: 620 }}>
            <h2 style={{ margin: 0, fontFamily: "var(--font-heading)", fontSize: 40, fontWeight: 800, letterSpacing: "-0.02em", color: "var(--white)", textWrap: "balance" }}>Need a fluid engineering partner?</h2>
            <p style={{ margin: 0, fontSize: 18, lineHeight: 1.6, color: "rgba(255,255,255,0.9)" }}>Request a technical consultation and our engineering team will respond within one business day.</p>
          </div>
          <div style={{ position: "relative", display: "flex", gap: 12 }}>
            <Button size="lg" onClick={() => onNavigate("Contact")} style={{ background: "var(--white)", color: "var(--navy-800)", border: "none" }} iconRight="arrow-right">Contact Us</Button>
          </div>
        </div>
      </Section>
    </div>
